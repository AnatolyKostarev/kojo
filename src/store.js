// Array of object for Work.jsx component
export const worksItem = [
  {
    imagePosition: "left",
    buttonPosition: "right",
    actionPosition: "right",
    title: "SaintCode Bootcamp",
    counter: "наша гордость #",
    desc: "сайт школы по обучению программированию",
    action: "дизайн & код",
    href: "https://saintcode.ru",
    src: "/img/works/image_works-1.webp",
    width: "840",
    height: "830",
    alt: "SaintCode Bootcamp",
  },
  {
    imagePosition: "right",
    buttonPosition: "left",
    actionPosition: "left",
    title: "Фронтирс",
    counter: "наша работа #",
    desc: "сайт инвестиционной компании",
    action: "код",
    href: "http://frontiers.ru",
    src: "/img/works/image_works-2.webp",
    width: "720",
    height: "720",
    alt: "Инвестиционная компания Фронтирс в Санкт-Петербурге",
  },
  {
    imagePosition: "left",
    buttonPosition: "right",
    actionPosition: "right",
    title: "Кино и Свет",
    counter: "наша работа #",
    desc: "сайт кинооборудования",
    action: "код & дизайн & seo",
    href: "https://kinoisvet.ru",
    src: "/img/works/image_works-3.webp",
    width: "840",
    height: "830",
    alt: "Cтудия KOJO",
  },
];

// Array of object for Navigation.jsx component
export const navSideBar = [
  { linkto: "/", title: "Главная" },
  { linkto: "/works", title: "Работы" },
  { linkto: "/services", title: "Услуги" },
  { linkto: "/contacts", title: "Контакты" },
];

// Array of object for Footerworks.jsx component

export const footerWorksNav = [
  {
    social: "/ facebook",
    href: "https://www.facebook.com/Saint-Code-Bootcamp-114797186729898",
  },
  {
    social: "/ instagram",
    href: "https://www.instagram.com/saint_code_bootcamp",
  },
];

// Array of object for Contacts.jsx component

export const socialList = [
  {
    social: "facebook",
    href: "https://www.facebook.com/Saint-Code-Bootcamp-114797186729898",
  },
  {
    social: "instagram",
    href: "https://www.instagram.com/saint_code_bootcamp",
  },
];

export const contactsList = [
  {
    label: "основной",
    href: "mailto:kojo@saintcode.ru",
    email: "kojo@saintcode.ru",
  },

  {
    label: "телеграм",
    href: "https://t.me/saintcoder",
    email: "@saintcoder",
  },
];

// Array of titles for ServicesInfo.jsx
export const servicesInfo = [
  {
    title: "Мы делаем сайты",
  },
  {
    title:
      "Предпочитаем писать фронт на React. " +
         "А бэк на Node/Express или PHP/Laravel",
  },
  {
    title: "Если нужен Wordpress, то мы используем Gatsby/GraphQL/React",
  },
  {
    title: "Увеличаем скорость работы сайта, как следствие " +
        "больше довольных пользователей и " +
        "более высокие позиции в поиске Google и Yandex. " +
        "(Чаще всего для этого нужно перенести сайт с конструктора Tilda, Wix, " +
        "Squarespace или CMS в чистый HTML/CSS/JS/React",
  },
  {
    title: "Делаем интересные анимации, как на этом сайте."
  },
];

// Array of titles for ServicesList.jsx

export const servicesList = [
  {
    title: "лэндингы",
  },
  {
    title: "промо сайты",
  },
  {
    title: "брэндовые сайты",
  },
  {
    title: "корпоративные сайты",
  },
  {
    title: "готовы рассмотреть интернет-магазины",
  },
];

// Array of titles for ServiceRulesInfo.jsx

export const servicesRulesInfo = [
  {
    title:
      "Как мы не работаем?",
  },
];

// Array of titles for ServiceRulesList.jsx

export const serviceRulesList = [
  {
    title: "бесплатно",
  },
  {
    title: "без утвержденного технического задания",
  },
  {
    title: "без предоплаты",
  },
];

// Array of titles for AboutUs.jsx

export const aboutUsInfo = [
  { label: "страна", value: "россия" },
  { label: "год", value: "2022" },
  { label: "тип", value: "веб студия" },
  { label: "акцент", value: "на важном" },
  { label: "цвета", value: "#fff, #000" },
];

export const awards = [
  { label: "второй", value: "Вникаем в суть, по-другому неинтересно" },
  { label: "третий", value: "Все задачи делаем измеримыми и понятными" },
  { label: "четвёртый", value: "Остаёмся людьми" },
];

// Array of src for title.jsx

export const homepageBackgroundImages = [
  "./img/imgHomepage/1Home.webp",
  "./img/imgHomepage/2Home.webp",
  "./img/imgHomepage/3Home.webp",
  "./img/imgHomepage/4Home.webp",
  "./img/imgHomepage/5Home.webp",
  "./img/imgHomepage/10Home.webp",
  "./img/imgHomepage/14Home.webp",
];

export const showServices = {
  display: "block",
};

export const hideServices = {
  display: "none",
};
